...
/*   Updated: 2025/07/29 12:25:04 by ybutkov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*read_from_fd(int fd, char *res, int *cur_str_size, char *buffer);
...
char	*get_next_line(int fd)
{
	...
	if ((fd < 0 || fd >= FD_MAX_SIZE) && read(fd, 0, 0) < 0)
		return (NULL);
	res = NULL;
	total_len = 0;
	if (buffer[fd][0])
...
static char	*read_from_fd(int fd, char *res, int *cur_str_size, char *buffer)
{
	...
	while (read_bytes > 0)
	{
		res = fill_str_from_buffer(res, cur_str_size, buffer);
		if (!res)
			return (NULL);
		if (res[*cur_str_size - 1] == '\n')
			break ;
...
static char	*fill_str_from_buffer(char *cur_str, int *total_len, char *buffer)
{
	...
	len = 0;
	res = NULL;
	while (buffer[len] && len < BUFFER_SIZE && buffer[len] != '\n')
		len++;
	if (buffer[len] == '\n')
		len += 1;
	if (len == 0)
